...
# ---------------------------------------------------------
def main():
    test_games = get_team_game_log_apiweb("DAL")
    print("Total games:", len(test_games))
    print("Last 3:", test_games[-3:])

    import requests, json

    # Pick any team
    TEAM = "DAL"
    schedule_url = f"https://api-web.nhle.com/v1/club-schedule-season/{TEAM}/20252026"

    sched = requests.get(schedule_url).json()

    # Find the first finished game
    game_id = None
    ...
    # -----------------------------
    # EXPANDERS FOR EACH MATCHUP
    # -----------------------------
    for g in games:
        away = g["away_abbr"]
        home = g["home_abbr"]

        with st.expander(f"{away} @ {home} — Full Team Stats"):
            st.subheader(f"{away} (Away Team)")
            away_stats = format_team_stats(team_stats.get(away, {"error": "No stats"}))
            st.json(away_stats)

            st.subheader(f"{home} (Home Team)")
            home_stats = format_team_stats(team_stats.get(home, {"error": "No stats"}))
            st.json(home_stats)


            # Pace stats if available
            if "SF/G" in away_stats:
                st.write(f"Shots For/G: {away_stats['SF/G']}, Shots Against/G: {away_stats['SA/G']}")
                st.write(f"Pace (SF+SA): {away_stats['Pace (SF+SA)']}")
